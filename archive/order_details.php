...
<?php
		...
		if(isset($_POST["confirm"]))
		{
		    ...
		    $query .= " product_id,price,quantity,total_price,user_id,fname,lname,mobile,email,address,pincode,city,state ";
		    $query .= ") VALUES (";
			$query .= " {$_POST['product_id']},{$price},{$_POST['qty']},{$total_price},{$_SESSION["user_id"]},'{$fname}','{$lname}','{$mobile}','{$email}','{$address}',{$_POST['pincode']},'{$_POST['city']}','{$_POST['state']}' ";
		    $query .= ")";
		    $result = mysqli_query($connection, $query);
...
<body>
    <div class="container">
		...
		<form align="right" method="post" action="order_details.php">
			<input type="hidden" name="pincode" value="$_POST['pincode']" >
